...
{
    public partial class Expediente2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Ubicacion != "")
            {
                Forms.Reff reff = new Forms.Reff(Ubicacion);
                if (reff.ShowDialog() == DialogResult.OK)
                {
                    textDireccion.Text = reff.Retorno;
                }
            }
            else
            {
                Forms.Reff reff = new Forms.Reff(PlugData.Direccion.Ubicacion.ToString());
                if (reff.ShowDialog() == DialogResult.OK)
                {
                    textDireccion.Text = reff.Retorno;
                }
