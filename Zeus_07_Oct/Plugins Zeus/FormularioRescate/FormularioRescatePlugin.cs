...
namespace FormularioRescate
{
    public class FormularioRescatePlugin : IPlugin
    {
...
#endif
            // menu
            ts = new ToolStripMenuItem("RESCATE", null, new EventHandler(MenuClick)) { Name = "config" };
            ZeusWin.AgregarConfig(ts);

            // boton
            tb = new ToolStripButton("RESCATE", Resource1.ico1, ButtonClick)
            {
                Name = "fRescateBtn",
        ...
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            {
                case "fRescateBtn":
                    FormularioRescate f = new FormularioRescate();
                    f.ShowDialog();
