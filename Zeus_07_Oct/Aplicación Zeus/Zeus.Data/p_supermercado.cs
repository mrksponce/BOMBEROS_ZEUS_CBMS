...
public class p_supermercado
{
	...
	public void Insert(p_supermercado myp_supermercado)
	{
		...
		string reqSQL = "INSERT INTO p_supermercado (gid,id,sgc1,sgc2,geom) VALUES ("+myp_supermercado.gid+","+myp_supermercado.id+","+myp_supermercado.sgc1+",'"+myp_supermercado.sgc2+"','"+myp_supermercado.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE p_supermercado SET gid="+myp_supermercado.gid+",id="+myp_supermercado.id+",sgc1="+myp_supermercado.sgc1+",sgc2='"+myp_supermercado.sgc2+"',geom='"+myp_supermercado.the_geom+"' WHERE (gid="+myp_supermercado.gid+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
	...
	public p_supermercado getObjectp_supermercado(System.Int32 myID)
	{
		...
		CnxBase myBase = new CnxBase();
		string reqSQL = "SELECT gid,id,sgc1,sgc2,geom FROM p_supermercado WHERE (gid="+myID+")";
		try
		{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
