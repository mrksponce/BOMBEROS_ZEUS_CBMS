...
public class k_grifos
{
	...
	public void Insert(k_grifos myk_grifos)
	{
		...
		string reqSQL = "INSERT INTO k_grifos (gid,identifica,comuna_,direccion,geom) VALUES ("+myk_grifos.gid+","+myk_grifos.identifica+",'"+myk_grifos.comuna_+"','"+myk_grifos.direccion+"','"+myk_grifos.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE k_grifos SET gid="+myk_grifos.gid+",identifica="+myk_grifos.identifica+",comuna_='"+myk_grifos.comuna_+"',direccion='"+myk_grifos.direccion+"',geom='"+myk_grifos.the_geom+"' WHERE (gid="+myk_grifos.gid+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
