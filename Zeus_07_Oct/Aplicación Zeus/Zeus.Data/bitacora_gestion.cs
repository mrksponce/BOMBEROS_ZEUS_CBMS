...
namespace Zeus.Data
{
    ...
    public class bitacora_gestion
    {
        ...
        //### Insert ID Gestion
        public void InsertIdGestion(int idGestion, string strCarro)
        {
            CnxBase myBase = new CnxBase();
            int ret = 0;
            string reqSQL = "INSERT INTO bitacora_gestion_id (fecha, tipo_gestion, nom_carro) VALUES ('" + System.DateTime.Now + "'," + idGestion + ",'" + strCarro + "')";
            try
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL, myConn);
        ...
        //### Eliminar Generador de ID de Gestion
        public void DeleteIdGestion(int idDeGestion)
        {
            CnxBase myBase = new CnxBase();
            string reqSQL = "DELETE FROM bitacora_gestion_id WHERE id_gestion = " + idDeGestion + "";
            try
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL, myConn);
        ...
        //### Select ID de Gestión
        public int SelectIdGestion(int tipoGestion, string nomCarro)
        {
            CnxBase myBase = new CnxBase();
            int idG = 0;
            string reqSQL = "SELECT id_gestion FROM bitacora_gestion_id WHERE (tipo_gestion = " + tipoGestion + " AND nom_carro ='" + nomCarro + "')";
            try
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL, myConn);
                ...
                if (myReader.Read())
                {
                    idG = Convert.ToInt32(myReader[0]);
                }
                myBase.CloseConnection(myConn);
                return idG;
            }
