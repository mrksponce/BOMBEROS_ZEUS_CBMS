...
namespace Zeus.Data
{
    ...
    public class z_carros
    {
        ...
        public ArrayList[] obtenerCarrosVirtual(string carros)
        {
            var myBase = new CnxBase();
            var myD4MCnx = new CnxBase();
            ArrayList list = new ArrayList();
            ...
            ArrayList list_ranking = new ArrayList();
            ArrayList[] array_list = new ArrayList[5];
            //SELECT * FROM z_carros WHERE id_carro IN (SELECT id_carro FROM z_carros_virtual WHERE id_tipo_carro = 101) =>     id_tipo_carro IN (" + carros + ")";
            //string reqSQL = "SELECT id_carro, carro_x, carro_y, id_compania, ranking FROM z_carros WHERE id_carro IN (SELECT id_carro FROM z_carros_virtual WHERE id_grupo = " + carros + ") ORDER BY id_carro"; 
            string reqSQL = "SELECT a.id_carro, a.carro_x, a.carro_y, a.id_compania, a.ranking, b.rx_2_q, b.id_grupo FROM z_carros a inner join z_carros_virtual b on a.id_carro = b.id_carro WHERE a.id_carro IN (SELECT id_carro FROM z_carros_virtual WHERE id_grupo = " + carros + ") and b.id_grupo = " + carros + " ORDER BY a.id_carro";
            try
            {
                DataSet myResult = myD4MCnx.GetDataSet(reqSQL);
                foreach (DataRow r_ocarros in myResult.Tables[0].Rows)
                {
                    ...
                    list_carro_y.Add(r_ocarros["carro_y"].ToString());
                    list_compania.Add(r_ocarros["id_compania"].ToString());
                    list_ranking.Add(1);  //(r_ocarros["ranking"].ToString());
                }
        ...
        public ArrayList[] obtenerCarrosVirtualPorClave(string carros)
        {
            var myBase = new CnxBase();
            var myD4MCnx = new CnxBase();
            ArrayList list = new ArrayList();
            ...
            //-+- Para 2-0 Sólo B y no BX.
            //if (carros == "1")
            //{
            //    reqSQL = "SELECT a.id_carro, a.carro_x, a.carro_y, a.id_compania, a.ranking, b.rx_2_q, b.id_grupo FROM z_carros a inner join z_carros_virtual b on a.id_carro = b.id_carro WHERE a.id_carro IN (SELECT id_carro FROM z_carros_virtual WHERE id_grupo = " + carros + " AND id_tipo_carro = 1) and b.id_grupo = " + carros + " ORDER BY a.id_carro";
            //}
            //else
            //{
        ...
        */
        ...
        public int ActualizarCoordenadasCubrirCuartel(int id_carro, int carro_x, int carro_y)
        {
            ...
            string reqSQL = "UPDATE z_carros SET carro_x = " + carro_x + ", carro_y = " + carro_y + ", estado = 1, carros_011 = true, evento = 'IDA' WHERE id_carro = " + id_carro;
            try
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL, myConn);
        ...
        public int Actualizar011_Retornar(int id_carro, int carro_x, int carro_y)
        {
            ...
            string reqSQL = "UPDATE z_carros SET carro_x = " + carro_x + ", carro_y = " + carro_y + ", estado = 1, carros_011 = true, evento = 'RETORNO' WHERE id_carro = " + id_carro;
            try
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL, myConn);
        ...
        public DataSet GetCarrosReemplazo()
        {
            ...
            string reqSQL = "SELECT id_virtual as virtual, id_carro as idcarro, nombre as nombrecarro, rx_2_q as boolreemplazo, reemplaza as carroreemplaza FROM z_carros_virtual WHERE (id_grupo = 2 and id_tipo_carro = 14) OR (id_grupo = 1 and id_tipo_carro = 15) OR (id_grupo = 1 and id_tipo_carro = 4)";
            try
            {
                myResult = myD4MCnx.GetDataSet(reqSQL);
            }
        ...
        */
        ...
        public DataSet Estado_Carro()
        {
            ...
            reqSQL += " FROM z_carros left JOIN z_estado_carros ON (z_carros.estado = z_estado_carros.id_estado) left JOIN z_conductores ON (z_carros.id_conductor = z_conductores.id_conductor) ORDER BY z_carros.id_compania, z_carros.nombre";
        ...
        {
            CnxBase myBase = new CnxBase();
            string reqSQL = "SELECT nombre, estado, id_compania_orig FROM z_carros ORDER BY estado, id_compania_orig, nombre ";
            try
            {
                CnxBase myD4MCnx = new CnxBase();
                DataSet myResult = myD4MCnx.GetDataSet(reqSQL);
