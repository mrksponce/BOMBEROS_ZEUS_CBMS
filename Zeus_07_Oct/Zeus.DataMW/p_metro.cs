...
public class p_metro
{
	...
	public void Insert(p_metro myp_metro)
	{
		...
		string reqSQL = "INSERT INTO p_metro (gid,id,linea,sgc1,sgc2,the_geom) VALUES ("+myp_metro.gid+","+myp_metro.id+",'"+myp_metro.linea+"',"+myp_metro.sgc1+",'"+myp_metro.sgc2+"','"+myp_metro.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE p_metro SET gid="+myp_metro.gid+",id="+myp_metro.id+",linea='"+myp_metro.linea+"',sgc1="+myp_metro.sgc1+",sgc2='"+myp_metro.sgc2+"',the_geom='"+myp_metro.the_geom+"' WHERE (gid="+myp_metro.gid+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
	...
	public p_metro getObjectp_metro(System.Int32 myID)
	{
		...
		CnxBase myBase = new CnxBase();
		string reqSQL = "SELECT gid,id,linea,sgc1,sgc2,the_geom FROM p_metro WHERE (gid="+myID+")";
		try
		{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
