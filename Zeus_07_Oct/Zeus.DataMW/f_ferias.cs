...
namespace Zeus.Data
{
    ...
    public class f_ferias
    {
        ...
        public void Delete(int myID)
        {
            ...
            {
                NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
                NpgsqlCommand myCommand = new NpgsqlCommand("DELETE FROM f_ferias WHERE (the_geom = " + myID + ")", myConn);
                myCommand.ExecuteNonQuery();
                myBase.CloseConnection(myConn);
            }
            catch (Exception myErr)
        ...
        public DataSet Getf_ferias_punto(PointD centro, int radio)
        {
            ...
            string reqSQL = "SELECT comuna, nombre, desde, hasta, d1, d2, hora_inicio, hora_termino, Distance(the_geom,GeometryFromText('POINT(" + centro.ToString() + ")',-1)) As radio, " +
                    "astext(the_geom) FROM f_ferias " +
                    "WHERE the_geom && setsrid('BOX3D(" + x1.ToString() + "," + x2.ToString() + ")'::box3d,-1) " +
                    "AND Distance(GeometryFromText('POINT(" + centro.ToString() + ")',-1),the_geom) < " + radio.ToString() + " ORDER BY radio";
            try
            {
                CnxBase myD4MCnx = new CnxBase();
                DataSet myResult = myD4MCnx.GetDataSet(reqSQL);
