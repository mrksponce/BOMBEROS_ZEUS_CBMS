...
public class k_geoz
{
	...
	public void Insert(k_geoz myk_geoz)
	{
		...
		string reqSQL = "INSERT INTO k_geoz (gid,id_hoja,id_celda,fila,columna,geoz,the_geom) VALUES ("+myk_geoz.gid+","+myk_geoz.id_hoja+",'"+myk_geoz.id_celda+"','"+myk_geoz.fila+"',"+myk_geoz.columna+",'"+myk_geoz.geoz+"','"+myk_geoz.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE k_geoz SET gid="+myk_geoz.gid+",id_hoja="+myk_geoz.id_hoja+",id_celda='"+myk_geoz.id_celda+"',fila='"+myk_geoz.fila+"',columna="+myk_geoz.columna+",geoz='"+myk_geoz.geoz+"',the_geom='"+myk_geoz.the_geom+"' WHERE (gid="+myk_geoz.gid+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
	...
	public k_geoz getObjectk_geoz(System.Int32 myID)
	{
		...
		CnxBase myBase = new CnxBase();
		string reqSQL = "SELECT gid,id_hoja,id_celda,fila,columna,geoz,the_geom FROM k_geoz WHERE (gid="+myID+")";
		try
		{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
