...
public class k_areas
{
	...
	public void Insert(k_areas myk_areas)
	{
		...
		string reqSQL = "INSERT INTO k_areas (gid,id_area,count,the_geom) VALUES ("+myk_areas.gid+","+myk_areas.id_area+","+myk_areas.count+",'"+myk_areas.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE k_areas SET gid="+myk_areas.gid+",id_area="+myk_areas.id_area+",count="+myk_areas.count+",the_geom='"+myk_areas.the_geom+"' WHERE (id_area="+myk_areas.id_area+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
