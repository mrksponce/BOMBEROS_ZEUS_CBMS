...
public class p_policia
{
	...
	public void Insert(p_policia myp_policia)
	{
		...
		string reqSQL = "INSERT INTO p_policia (gid,sgc1,sgc2,the_geom) VALUES ("+myp_policia.gid+","+myp_policia.sgc1+",'"+myp_policia.sgc2+"','"+myp_policia.the_geom+"')";
			try
			{
	...
	{
		CnxBase myBase = new CnxBase();
		string reqSQL = "UPDATE p_policia SET gid="+myp_policia.gid+",sgc1="+myp_policia.sgc1+",sgc2='"+myp_policia.sgc2+"',the_geom='"+myp_policia.the_geom+"' WHERE (gid="+myp_policia.gid+")";
			try
			{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
	...
	public p_policia getObjectp_policia(System.Int32 myID)
	{
		...
		CnxBase myBase = new CnxBase();
		string reqSQL = "SELECT gid,sgc1,sgc2,the_geom FROM p_policia WHERE (gid="+myID+")";
		try
		{
				NpgsqlConnection myConn = myBase.OpenConnection(myBase.cnxString);
				NpgsqlCommand myCommand = new NpgsqlCommand(reqSQL,myConn);
