...
namespace LegendControl
{
	...
	public class Layer
	{
		...
		private object NewColorLegend
		{
			set
			{
				...
				if(ColorScheme != null)
				{
					...
					MapWinGIS.ShapefileColorScheme sfcs = null;
					//if(ColorSchemeType == typeof(MapWinGIS.ShapefileColorScheme) || ColorSchemeType == typeof(MapWinGIS.ShapefileColorSchemeClass))
					try
					{
						sfcs = (MapWinGIS.ShapefileColorScheme)ColorScheme;
					}
					...
					{
						MapWinGIS.GridColorScheme  gcs = null;

						//if(ColorSchemeType == typeof(MapWinGIS.GridColorScheme) || ColorSchemeType == typeof(MapWinGIS.GridColorSchemeClass))
						try
						{
							gcs = (MapWinGIS.GridColorScheme) ColorScheme;
						}
						catch(System.InvalidCastException ex)
						{
							string str = ex.Message;
				...
				this.m_Height = CalcHeight();//update the height, it may have changed
			}
		...
		/// </summary>
		/// <param name="UseExpanededHeight">If True, the height returned is the expanded height. Otherwise, the height is the displayed height of the layer</param>
		/// <returns>Height of layer(depends on 'Expanded' state of the layer)</returns>
		protected internal int CalcHeight(bool UseExpanededHeight)
		{
            ...
            // Our own calculation
            int ret = 0;
			if (UseExpanededHeight == false && (m_Expanded == false || ColorLegend.Count == 0))
            ...
            if ((UseExpanededHeight || m_Expanded == true) && (HatchingScheme != null && HatchingScheme.NumHatches() > 0))
				ret += Constants.ITEM_HEIGHT + (HatchingScheme.NumHatches() * Constants.CS_ITEM_HEIGHT) + 2;
		...
		/// <summary>
		/// Gets the Actual(when drawn) height of the control
		/// </summary>
