	...
	public class Legend : System.Windows.Forms.UserControl
	{
		...
		protected internal bool RemoveGroup(int Handle)
		{
			...
			if(IsValidGroup(Handle) == true)
			{
                ...
                //---Cho 1/2/2009: Update m_GroupPositions too.
                m_GroupPositions.Remove(index);
				...
				for(int i = ItemCount-1; i>=0; i--)
				{
					...
					if (!lyr.HideFromLegend)
					{
						...
						if(IsSnapshot == false)
						{
							...
							if(lyr.ColorLegend.Count == 0 && lyr.Handle != m_SelectedLayerHandle)
		...
		{
			int rslt = AddLayer(newLayer, Visible);
			if (!PlaceAboveCurrentlySelected) return rslt;
			...
			if (SelectedLayer != -1 && PlaceAboveCurrentlySelected)
			{
				...
				Layers.MoveLayer(rslt, addGrp, addPos);
				return rslt;
			}
		...
		internal bool HasTransparency(object newLayer)
		{
			try
			{
				MapWinGIS.Image img = (MapWinGIS.Image)newLayer;
		...
		private eLayerType GetLayerType(object newLayer)
		{
			...
			if(img != null)
			{
				...
				//else if(type.ToString() == "MapWinGIS.Shapefile")
		...
		protected internal void DrawLayer(Graphics DrawTool, Layer lyr, Rectangle bounds, bool IsSnapshot)
		{
            ...
            int p = 0;
            if (!Handled && lyr.ColorLegend.Count > 0)
            {
                if ((IsSnapshot == true || lyr.Expanded == true) && bounds.Width > 47)
                {
                    ...
                    for (; p < NumBreaks; p++)
                    {
                        ...
                        if (ci.IsTransparent == false)
                        {
                            if (lyr.Type == eLayerType.Grid || lyr.Type == eLayerType.Image)
                            {
                                DrawColorPatch(DrawTool, ci.StartColor, ci.EndColor, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, bounds.Left + Constants.CS_PATCH_LEFT_INDENT, Constants.CS_PATCH_HEIGHT, Constants.CS_PATCH_WIDTH, System.Drawing.ColorTranslator.FromOle(Convert.ToInt32(m_Map.get_ShapeLayerLineColor(lyr.Handle))), true);
                            }
                            else
                            {
                                System.Drawing.Color sc = System.Drawing.Color.FromArgb((int)(m_Map.get_ShapeLayerFillTransparency(lyr.Handle) * 255), ci.StartColor);
                                System.Drawing.Color ec = System.Drawing.Color.FromArgb((int)(m_Map.get_ShapeLayerFillTransparency(lyr.Handle) * 255), ci.EndColor);
                                DrawColorPatch(DrawTool, sc, ec, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, bounds.Left + Constants.CS_PATCH_LEFT_INDENT, Constants.CS_PATCH_HEIGHT, Constants.CS_PATCH_WIDTH, System.Drawing.ColorTranslator.FromOle(Convert.ToInt32(m_Map.get_ShapeLayerLineColor(lyr.Handle))), (m_Map.get_ShapeLayerLineWidth(lyr.Handle) != 0), lyr.Type);
                            }
                        }
                        else
                        {
                            DrawTransparentPatch(DrawTool, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, bounds.Left + Constants.CS_PATCH_LEFT_INDENT, Constants.CS_PATCH_HEIGHT, Constants.CS_PATCH_WIDTH, System.Drawing.ColorTranslator.FromOle(Convert.ToInt32(m_Map.get_ShapeLayerLineColor(lyr.Handle))), (m_Map.get_ShapeLayerLineWidth(lyr.Handle) != 0));
                        }

                        rect = new Rectangle(bounds.Left + Constants.CS_TEXT_LEFT_INDENT, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, bounds.Width - Constants.TEXT_RIGHT_PAD_NO_ICON - Constants.CS_TEXT_LEFT_INDENT, Constants.TEXT_HEIGHT);
            ...
            if (!Handled && lyr.HatchingScheme != null && lyr.HatchingScheme.NumHatches() > 0)
            {
                if ((IsSnapshot == true || lyr.Expanded == true) && bounds.Width > 47)
                {
                    ...
                    while (i.MoveNext())
                    {
                        Rectangle fillRect = new Rectangle(bounds.Left + Constants.CS_PATCH_LEFT_INDENT, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, Constants.CS_PATCH_WIDTH, Constants.CS_PATCH_HEIGHT);
                        ...
                        DrawTool.DrawRectangle(pen, bounds.Left + Constants.CS_PATCH_LEFT_INDENT, bounds.Top + Constants.ITEM_HEIGHT + p * Constants.CS_ITEM_HEIGHT, Constants.CS_PATCH_WIDTH, Constants.CS_PATCH_HEIGHT);
				...
				for(int j = 0; j < LayerCount; j++)
				{
					...
					if( bounds.Contains(point) == true)
					{
						...
						else
						{
							...
							bounds = new Rectangle(CurLeft,CurTop,CurWidth,CurHeight);
							if(bounds.Contains(point) == true && (lyr.ColorLegend.Count > 0 || lyr.ExpansionBoxForceAllowed || lyr.ColorLegend.Count > 0 || (lyr.HatchingScheme != null && lyr.HatchingScheme.NumHatches() > 0)))
							{
		...
		private void HandleLeftMouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if(grp != null)
			{
				if(InCheckBox == true)
				{
                    if (!grp.StateLocked)
                    {
                        ...
                        if (GroupCheckboxClicked != null)
                            try
                            {
                                GroupCheckboxClicked(grp.Handle, grp.VisibleState);
                            }
                            catch
                            {
				...
				else if(InExpandBox == true)
				{
					...
					GroupExpandedChanged(grp.Handle, grp.Expanded);
					Redraw();
					return;
				}
				else
			...
			if(lyr != null)
			{
				...
				if(InCheckBox == true)
				{
                    ...
                    LayerCheckboxClicked(lyr.Handle, NewState);
					Redraw();
					return;
				}
				else if(InExpandBox == true)
