...
namespace MapWinGeoProc
{
	...
	public class Utils
	{
		...
		/// <returns>False if an error was encountered, true otherwise.</returns>
		public static bool FindNearestPointAndLoc(ref MapWinGIS.Point testPoint, ref MapWinGIS.Shape shape, ref MapWinGIS.Point resultPoint, out int ptLocation, out double ptDistance)
		{
            double shortPointX, shortPointY, shortDist, currDist;
            Topology2D.Point currClosest;

            Topology2D.Point geomPoint = new MapWinGeoProc.Topology2D.Point(testPoint.x, testPoint.y);
            Topology2D.LineString geomLine = new MapWinGeoProc.Topology2D.LineString(shape);
            System.Collections.Generic.List<Topology2D.LineSegment> geomSegs = geomLine.ToLineSegments();

            shortDist = 100000;
            shortPointX = -1;
            shortPointY = -1;
            for (int i = 0; i < geomSegs.Count; i++)
            {
                currClosest = geomPoint.ClosestPointOn(geomSegs[i]);
                if (currClosest == null)
                {
                    shortDist = 0.0;
                ...
                {
                    currDist = Math.Sqrt((currClosest.X - geomPoint.X) * (currClosest.X - geomPoint.X) + (currClosest.Y - geomPoint.Y) * (currClosest.Y - geomPoint.Y));
        ...
        /// <param name="ptDistance">The distance from the test point to the closest point.</param>
        /// <returns>False if an error was encountered, true otherwise.</returns>
        public static bool FindNearestPointAndLocOld(ref MapWinGIS.Point testPoint, ref MapWinGIS.Shape shape, ref MapWinGIS.Point resultPoint, out int ptLocation, out double ptDistance)
        {
            MapWinGIS.Shapefile debugshp = new MapWinGIS.Shapefile();

            double segLength, riseStep, runStep, dStepSize;
            double currPointX, currPointY, testPointY, testPointX;
            double shortPointX, shortPointY, shortDist, currDist;
            int loopCount, tmpLoc;
            double dX, dY;

            MapWinGIS.Point tmpPt1;
            MapWinGIS.Point tmpPt2;
            dStepSize = 0.1;
            shortDist = 1000000;
            ...
            shortPointY = -1;
            tmpLoc = 0;
            testPointX = testPoint.x;
            testPointY = testPoint.y;
            ...
            {
                tmpPt1 = shape.get_Point(i - 1);
                tmpPt2 = shape.get_Point(i);
                dX = (tmpPt2.x - tmpPt1.x);
                dY = (tmpPt2.y - tmpPt1.y);
                segLength = Math.Sqrt(Math.Pow(dX, 2) + Math.Pow(dY, 2));
                runStep = dStepSize * (tmpPt2.x - tmpPt1.x) / segLength;
                riseStep = dStepSize * (tmpPt2.y - tmpPt1.y) / segLength;
                loopCount = int.Parse(Math.Round(segLength / dStepSize).ToString());
        ...
        private static bool doSnapPointsToLines(string PointsPath, string LinesPath, double SnapThreshold, double ExclusionThreshold, string SnapResultPath, bool CopyAttributes, MapWinGIS.ICallback Callback)
        {
            MapWinGIS.Shapefile pt = new MapWinGIS.Shapefile();
            MapWinGIS.Shapefile ln = new MapWinGIS.Shapefile();
            MapWinGIS.Shapefile newPoint = new MapWinGIS.Shapefile();
            MapWinGIS.Point resPt = new MapWinGIS.Point();
            MapWinGIS.Point lowestPt = new MapWinGIS.Point();
            MapWinGIS.Point currPt;
            MapWinGIS.Shape currLn;
            MapWinGIS.Shape tmpPointShape;
            int resLoc;
            int newperc;
            int oldperc;
            int tmpPointCount;
            ...
            if (!(newPoint.CreateNew(SnapResultPath, MapWinGIS.ShpfileType.SHP_POINT)))
            {
                return false;
            }
            ...
            if (CopyAttributes)
            {
                ...
                for (int f = 0; f <= pt.NumFields - 1; f++)
                {
            ...
            for (int i = 0; i <= pt.NumShapes - 1; i++)
            {
                if (pt.NumShapes > 0)
                {
                    if (Callback != null)
                    {
                        newperc = Convert.ToInt32((Convert.ToDouble(i) / Convert.ToDouble(pt.NumShapes)) * 100);
                        if (newperc > oldperc)
                        {
                ...
                currPt = pt.get_Shape(i).get_Point(0); 
                
                for (int j = 0; j <= ln.NumShapes - 1; j++)
                {
                    currLn = ln.get_Shape(j);
                    //if (FindNearestPointAndLoc(ref currPt, ref currLn, ref resPt, out resLoc, out resDist))
                ...
                newPointCount = newPoint.NumShapes;
                tmpPointCount = 0;
                if (newPointCount == 0)
                {
                    if (lowestDist <= SnapThreshold)
                    {
                        tmpPointShape = new MapWinGIS.Shape();
                        tmpPointShape.Create(MapWinGIS.ShpfileType.SHP_POINT);
                        ...
                        if (CopyAttributes)
                        {
                            for (int f = 0; f <= pt.NumFields - 1; f++)
                            {
                                bool tmpbool = newPoint.EditCellValue(f, newPoint.NumShapes-1, pt.get_CellValue(f, i));
                            }
                        }
                    }
                }
                ...
                {
                    ptAlreadyAdded = false;
                    for (int j = 0; j <= newPoint.NumShapes - 1; j++)
                    {
                        MapWinGIS.Point tmpPt = newPoint.get_Shape(j).get_Point(0);
            ...
            if (newPoint.NumShapes == 0)
            {
                System.Windows.Forms.MessageBox.Show("After snapping, no outlets were within the threshold to be snapped. Please set a higher snap threshold, draw new outlets, or uncheck the Use Custom Outlets option, then rerun the delineation.");
                newPoint.Close();
                return false;
            }
            newPoint.Close();
        ...
        private static bool isPointInThreshold(double startPointX, double startPointY, double checkPointX, double checkPointY, double thresh)
        {
            ...
            double maxX;
            double maxY;
            minX = startPointX - thresh;
            maxX = startPointX + thresh;
            minY = startPointY - thresh;
            maxY = startPointY + thresh;
            if (checkPointX > minX & checkPointX < maxX & checkPointY > minY & checkPointY < maxY)
            {
                return true;
            }
            else
        ...
        public static bool ExtractSelectedPoints(string PointShapePath, string ExtactedPointShapeResultPath, ArrayList SelectedIndexesList, MapWinGIS.ICallback callback)
        {
            ...
            MapWinGIS.Shapefile pointsNew = new MapWinGIS.Shapefile();
            ...
            if (SelectedIndexesList.Count > 0)
            {
                if (SelectedIndexesList.Count != points.NumShapes)
                {
                    ...
                    for (int f = 0; f <= points.NumFields - 1; f++)
                    {
                        tmpField = new MapWinGIS.Field();
                    ...
                    {
                        tmpShape = points.get_Shape((int)SelectedIndexesList[i]);
                        tmpPointCount = points.NumShapes;
                        pointsNew.EditInsertShape(tmpShape, ref tmpPointCount);
                        for (int f = 0; f <= points.NumFields - 1; f++)
                        {
                            pointsNew.EditCellValue(f, pointsNew.NumShapes - 1, points.get_CellValue(f, (int)SelectedIndexesList[i]));
                        }
                    }
