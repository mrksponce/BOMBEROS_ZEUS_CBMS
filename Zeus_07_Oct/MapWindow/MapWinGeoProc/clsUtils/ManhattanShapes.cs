...
namespace MapWinGeoProc
{
	...
	public class ManhattanShapes
	{
		...
		private class Link
		{
			...
			/// <param name="l2"></param>
			/// <returns></returns>
			public static bool connected(Link l1, Link l2)
			{
				int x1, y1, x2, y2;
				l1.finish(out x1, out y1);
				l2.start(out x2, out y2);
				return ((x1 == x2) && (y1 == y2));
			}
		}
		...
		private class Polygon
		{
			...
			public static Polygon box(int x, int y, int width)
			{
				...
				{
					perimeter.Add(new Link(x+i, y+1, Direction.right));
				}
				perimeter.Add(new Link(x+width, y, Direction.down));
				for (int i = width - 1; i >= 0; i--)
				{
					perimeter.Add(new Link(x + i, y, Direction.left));
				}
			...
			/// <summary>
			/// Makes a singe polygon from two polygons p1 and p2 with complementary
			...
			public static bool hasHole(List<Link> l, out int first, out int last)
			{
				for (first = 0; first < l.Count - 1; first++)
				{
					...
					if ((dir == Direction.left) || (dir == Direction.right))
					{
						last = l.FindIndex(first+1, link.complements);
			...
			public static string makeString(List<Link> l)
			{
				...
				{
					next = l[i];
					if (Link.connected(current, next))
					{
						current = next;
						res += current.dc();
					}
		...
		public bool InsertShape(MapWinGIS.Shapefile shapeFile, int val, ref int shapeIndex)
		{
			...
			else return shapeFile.EditInsertShape(shape, ref shapeIndex);
		}
		                                                       ...
		                                                       string id, MapWinGIS.ICallback cb)
		{
			...
			{
				if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.noopengrid + gridName);
				return null;
			}
			...
			if (!success)
			{
				if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.nocreateshapefile + shapeName);
				return null;
			}
			success = shapeFile.StartEditingShapes(true, null);
			if (!success)
			{
				if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.noeditshapefile + shapeName);
				return null;
			}
			...
			if (!success)
			{
				if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.noaddfield + shapeName);
				shapeFile.Close();
				return null;
			}
			...
			foreach (int k in shapes.ShapesTable.Keys)
			{
				...
				if (!success)
				{
					if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.noaddshape + shapeName);
					shapeFile.Close();
					return null;
				}
				...
				if (!success)
				{
					if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.noeditcell + shapeName);
					shapeFile.Close();
					return null;
				}
			...
			if (!shapeFile.Close())
			{
				if (cb != null) cb.Error("GridToShapeManhattan", clsUtils.ManhattanShapes.nocloseshapefile + shapeName);
				return null;
			}
			return shapeFile;
		}
