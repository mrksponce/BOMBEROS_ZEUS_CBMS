...
namespace MapWinGeoProc
{
	...
	public class SpatialReference
	{
		...
		public static bool ProjectGrid(ref string sourceProj, ref string destProj, ref string inputGrid, ref string resultGrid, bool trimResult)
		{
            ...
            return file_ProjectGrid(ref sourceProj, ref destProj, ref inputGrid, ref resultGrid, trimResult, null);
        ...
        public static bool ProjectGrid(ref string sourceProj, ref string destProj, ref string inputGrid, ref string resultGrid, bool trimResult, MapWinGIS.ICallback ICallBack)
        {
            MapWinUtility.Logger.Dbg("ProjectGrid(sourceProj: " + sourceProj + ",\n" +
                                     ...
                                     "            trimResult: " + trimResult.ToString() + ",\n" +
                                     "            ICallback)");
            return file_ProjectGrid(ref sourceProj, ref destProj, ref inputGrid, ref resultGrid, trimResult, ICallBack);
        }
        ...
        /// <param name="dstproj">Destination projection (proj4 format)</param>
         public static void ReprojectLabels(string src, string dst, string srcproj, string dstproj)
         {
             ...
             System.Text.RegularExpressions.Regex xReg = new System.Text.RegularExpressions.Regex("X=\"[0-9]\\.[0-9]\"");
             System.Text.RegularExpressions.Regex yReg = new System.Text.RegularExpressions.Regex("Y=\"[0-9]\\.[0-9]\"");
		...
		private static bool file_ProjectGrid(ref string sourceProj, ref string destProj, ref string inputGrid, ref string resultGrid, bool trimResult, MapWinGIS.ICallback ICallBack)
		{
            MapWinUtility.Logger.Dbg("file_ProjectGrid(sourceProj: " + sourceProj + ",\n" +
                                     "            destProj: " + destProj + ",\n" +
                                     "            inputGrid: " + inputGrid + ",\n" +
                                     "            resultGrid: " + resultGrid + ",\n" +
                                     "            trimResult: " + trimResult.ToString() + ",\n" +
			...
			bool status = true;
			bool unitsChanged = false; // Flag to set if distances in old and new grids are incomparable
									   // Added Chris George 14 October 2007
			bool toSmaller = false;
			...
			try
			{
				...
				System.IO.FileInfo fileInfo = new FileInfo(inputGrid);
				//long fileSize = fileInfo.Length; //size of file in bytes
				string fileEXT = fileInfo.Extension;
				Debug.WriteLine("inputGrid extension: " + fileEXT);
				string tempPath = System.IO.Path.GetTempPath() + "tempGrid" + fileEXT;
				Debug.WriteLine("tempPath = " + tempPath);
				string tempGFPath = tempPath;
				
				MapWinGIS.Grid grid = new MapWinGIS.GridClass();
				if(grid.Open(inputGrid, MapWinGIS.GridDataType.UnknownDataType, inRAM, MapWinGIS.GridFileType.UseExtension, null)== false)
				{
				...
				double oldXll = grid.Header.XllCenter;
				double oldYll = grid.Header.YllCenter;
				double oldSize = grid.Header.dX;
				double oldDx = grid.Header.dX;
				double oldDy = grid.Header.dY;
				...
				if(status == true)
				{
					...
					//grid.CellToProj(numCols-1, 0, out x, out y);
					...
					if(status == true)
					{
						...
						//							x = oldXll + oldSize * (numCols-1) + oldSize/2;
						//							y = oldYll - oldSize/2;
						//grid.CellToProj(numCols-1, numRows-1, out x, out y);
						...
						if(status == true)
						{
							...
							//								x = oldXll - oldSize/2;
							//								y = oldYll - oldSize/2;
							//grid.CellToProj(0, numRows-1, out x, out y);
				...
				if (!unitsChanged)
				{
					double oldDist = Distance(oldX1, oldX2);
					double newDist = Distance(LTptX, RTptX);
					if(oldDist > newDist)
					{
						toSmaller = true;
					}
				...
				if(unitsChanged || (toSmaller == true))
				{
					...
					if (!unitsChanged)
					{
						double dX = grid.Header.dX;
						double dY = grid.Header.dY;
						double diff = Distance(dX, width);
						if(diff > 0 && diff < 1)
						{
							cellWidth = dX;
						}
				...
				else //toSmaller == false
				{
					...
					if(oldSize < 1)
					{
						...
						double diff = Distance(newSize, defaultSize);
						if(diff > 0 && diff < 1)
						{
							newSize = defaultSize;
						}
				...
				int newNumCols = (int)((Distance(maxX, minX) / cellSize) + 0.5);
				int newNumRows = (int)((Distance(maxY, minY) / cellSize) + 0.5);
				//The point:(minX, minY) represents the left-bottom corner of the grid
				//shift accordingly to get the center point of the cell.
				double newXll = minX + cellSize/2;
				double newYll = minY + cellSize/2;
				...
				// Bad idea --> header = grid.Header;
				header.NumberCols = newNumCols;
				header.NumberRows = newNumRows;
				header.dX = cellSize;
				header.dY = cellSize;
				header.NodataValue = grid.Header.NodataValue;
				header.XllCenter = newXll;
				header.YllCenter = newYll;
				header.Projection = destProj;

				//Now that we have a header, create the new grid
				MapWinGIS.Grid resultGF = new MapWinGIS.GridClass();
				
				if(trimResult == true)//this grid is temporary
				{
					DataManagement.DeleteGrid(ref tempGFPath);							
					if(resultGF.CreateNew(tempGFPath, header, grid.DataType, grid.Header.NodataValue, true, MapWinGIS.GridFileType.UseExtension, null) == false)
					{
					...
					resultGF.Save(tempGFPath, MapWinGIS.GridFileType.UseExtension, null);
				}
				
				else //this grid is final
				{				
				...
				if(trimResult == true)
				{
					if(FillGrid(ref inputGrid, ref tempGFPath, newNumRows, newNumCols, rowClearCount, newXll, newYll, cellSize,
						oldXll, oldYll, oldDx, oldDy, ICallBack) == false)
					{
                        if (resultGF != null)
                            resultGF.Close();
                        if (grid != null)
					...
					if(Globals.TrimGrid(ref tempGFPath, ref resultGrid) == false)
					{
				...
				{
					if(FillGrid(ref inputGrid, ref resultGrid, newNumRows, newNumCols, rowClearCount, newXll, newYll, cellSize,
						oldXll, oldYll, oldDx, oldDy, ICallBack) == false)
					{
                        if (resultGF != null)
                            resultGF.Close();
			...
			double oldXll, double oldYll, double oldDx, double oldDy, MapWinGIS.ICallback ICallBack)
		{
			...
			for(int row = startRow; row >= 0; row--)
			{
				...
				for(int col = 0; col < newNumCols; col++)
				{
					...
					if(status == true)
					{
						...
						if(oldCol <= -1 || oldRow <= -1 || oldCol >= oldNumCols || oldRow >= oldNumRows)
						{
							...
							resultGW.set_Value(col, row, noDataValue);
						}
						else
						{
							resultGW.set_Value(col, row, inputGW.get_Value(oldCol, oldRow));
						}
					}
					else
					{
                ...
                if (ICallBack != null)
                {
                    int Prog = (int)(100*(startRow-row)/startRow);
                    if(Prog > OldProg)
                    {
        ...
        public static bool memory_ProjectGrid(ref string sourceProj, ref string destProj, ref MapWinGIS.Grid inputGrid, out MapWinGIS.Grid resultGrid, bool trimResult, MapWinGIS.ICallback ICallBack)
		{
			...
			int extStart = inputGrid.Filename.IndexOf(".", 0);
			string fileEXT = inputGrid.Filename.Substring(extStart);
			Debug.WriteLine("inputGrid extension: " + fileEXT);
			string tempPath = System.IO.Path.GetTempPath();
			string resultGridFile = tempPath + "tempGF1" + fileEXT;//@"C:\temp\resultGrid.bgd";
			...
			try
			{
				...
				{
					int numCols = inputGrid.Header.NumberCols;
					int numRows = inputGrid.Header.NumberRows;
					...
					double oldXll = inputGrid.Header.XllCenter;
					double oldYll = inputGrid.Header.YllCenter;
					double oldSize = inputGrid.Header.dX;
					...
					{
						double oldDist = Distance(oldX1, oldX2);
						double newDist = Distance(LTpt.x, RTpt.x);
						if(oldDist > newDist)
						{
							toSmaller = true;
						}
					...
					if(unitsChanged || (toSmaller == true))
					{
						...
						if (!unitsChanged)
						{
							double dX = inputGrid.Header.dX;
							double dY = inputGrid.Header.dY;
							double diff = Distance(dX, width);
							if(diff > 0 && diff < 1)
							{
								cellWidth = dX;
					...
					//calculate the number of cells needed to build the grid
					//converting to int rounds down, so add 0.5
					int colNum = (int)((Distance(maxX, minX) / cellSize) + 0.5);
					int rowNum = (int)((Distance(maxY, minY) / cellSize) + 0.5);
					...
					{
						string tempGrid = tempPath + "tempGF2" + fileEXT;//@"C:\Temp\tempGrid.bgd";
					...
					int oldCol, oldRow;
					double cPt = resultGF.Header.XllCenter; 
					double rPt = resultGF.Header.YllCenter;
                    int OldProg = -1;
					for(int row = rowNum-1; row >= 0; row--)
					{
                        ...
                        if (ICallBack != null)
                        {
                            int Prog = 100 * (rowNum - row) / rowNum;
                            if (Prog > OldProg)
                            {
                                if (Prog > 100) Prog = 100;
					...
					{
						//Now trim any excess rows and columns that contain values of only NoData
						double noData = Convert.ToDouble(resultGF.Header.NodataValue);
						int firstRowLoc = 0;
						int lastRowLoc = rowNum-1;
						int firstColLoc = 0;
						int lastColLoc = colNum-1;
						bool found = false;
                        ...
                        int newColNum = (colNum - firstColLoc) - (colNum - lastColLoc) + 1;
						int newRowNum = (rowNum - firstRowLoc) - (rowNum - lastRowLoc) + 1;
        ...
        public static void ProjectImage(string sourceProj, string destProj, string inputImage, string resultImage, MapWinGIS.ICallback ICallBack)
        {
            ...
            inLR.x = SourceImage.XllCenter + SourceImage.dX * SourceImage.Height;
            inUL = new MapWinGIS.Point();
            inUL.x = SourceImage.XllCenter;
            inUL.y = SourceImage.YllCenter + SourceImage.dY * SourceImage.Width;
            inUR = new MapWinGIS.Point();
            inUR.x = SourceImage.XllCenter + SourceImage.dX * SourceImage.Width;
            inUR.y = SourceImage.YllCenter + SourceImage.dY * SourceImage.Height;
            ...
            */
            ...
            string ext = System.IO.Path.GetExtension(resultImage);
            // This speed-up is only applicable to file types supported by 
            // System.Drawing.Image 
            if (ext == ".gif" || ext == ".jgp" || ext == ".bmp" || ext == ".png" || ext == ".tif")
            {

                Transforms.Affine Aff = new MapWinGeoProc.Transforms.Affine();
            ...
            Transforms.Projective Proj = new Transforms.Projective();
        ...
        public static void RectifyToWorldFile(string inputImage, string resultImage, MapWinGIS.ICallback ICallBack)
        {
            ...
            {
                string ext = System.IO.Path.GetExtension(inputImage);
            ...
            System.IO.StreamReader sr = new System.IO.StreamReader(worldfile);
            double a, b, c, d, e, f;
            ...
            MapWinGIS.Point outLL = new MapWinGIS.Point();
            MapWinGIS.Point outUL = new MapWinGIS.Point();
            MapWinGIS.Point outUR = new MapWinGIS.Point();
            MapWinGIS.Point outLR = new MapWinGIS.Point();
            outUL.x = c;
            outUL.y = f;
            ...
            Transforms.Projective proj = new MapWinGeoProc.Transforms.Projective();
		...
		public static bool ProjectShapefile(ref string sourceProj, ref string destProj, ref string inputSF, ref string resultSF)
		{
            ...
            string inputFile = MapWinUtility.modFile.FilenameNoExt(inputSF);
            string resultFile = MapWinUtility.modFile.FilenameNoExt(resultSF);
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i <= numShapes - 1; i++)
                    {
                        ...
                        if (numParts > 1)
                        {
                            ...
                            for (int j = 0; j <= numParts - 1; j++)
                            {
                                ...
                                for (int k = begPart; k <= endPart - 1; k++)
                                {
                    ...
                    //copy the .dbf table
                    string inputDBF = inputFile + ".dbf";
                    string resultDBF = resultFile + ".dbf";
        ...
        public static bool ProjectShapefile(ref string destProj, ref string inputSF, ref string resultSF)
        {
            ...
            MapWinGIS.Shapefile inputf = new MapWinGIS.Shapefile();
            inputf.Open(inputSF, null);
            if (inputSF == null) return false;
            string prj = inputf.Projection;
            inputf.Close();
		...
		#region private Distance(): avoids most overflow errors
		/// <summary>
		...
		private static bool ConvertAndProjectPoint(ref double x, ref double y)
		{
			...
			double Z = 0;
			int retcode = pj_transform(srcPrj, destPrj, 1, 0, ref x, ref y, ref Z);
        ...
        private static MapWinGIS.Point ConvertAndProjectPoint(MapWinGIS.Point InputPoint)
        {
            ...
            x = InputPoint.x;
            y = InputPoint.y;
            MapWinGIS.Point OutputPoint = new MapWinGIS.Point();
            if (convertToRadians == true)
            {
                x *= DEG_TO_RAD;
                y *= DEG_TO_RAD;
		...
		{
			string msg = "prj_transform error: ";
			if(retcode != 0)
			{
				switch(retcode)
				{
