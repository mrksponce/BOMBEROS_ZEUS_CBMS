...
namespace MapWinGeoProc
{
    ...
    public class TerrainAnalysis
    {
        ...
        private static bool doSlope(string inDEMPath, double zFactor, string outSlopePath, bool slopeInPercent, MapWinGIS.ICallback callback)
        {
            ...
            for (int row = 1; row < head.NumberRows - 1; row++)
            {
                Prog = (int)((row * 100) / (head.NumberRows));
                if (callback != null)
                {
                    if (Prog > OldProg)
                    {
                        callback.Progress("Progress", Prog, "Calculating Slope..." + Prog.ToString() + "% Complete");
                        MapWinUtility.Logger.Progress("Calculating Slope..." + Prog.ToString() + "% Complete", Prog, OldProg);
                        OldProg = Prog;
                    }
                }

                for (int col = 1; col < head.NumberCols - 1; col++)
                {
        ...
        /// Aspect uses modification of bolstad's aspect to match the ESRI specification at http://support.esri.com/index.cfm?fa=knowledgebase.techarticles.articleShow&d=21345
        ...
        public static bool SlopeAspectCurvature(string inDEMPath, float zFactor, string outSlopePath, bool slopeInPercent, string outAspectPath, string outProfileCurvePath, string outPlanCurvePath, MapWinGIS.ICallback callback)
        {
            ...
            for (int row = 1; row < nr - 1; row++)
            {
                if (callback != null)
                {
                    ...
                    MapWinUtility.Logger.Progress("Calc Window..." + Prog.ToString() + "% Complete", Prog, OldProg);
                }
        ...
        public static bool RunWindow(string inGridPath, List<MovingWindow> inWindows, MapWinGIS.ICallback callback)
        {
            ...
            if (!inGrid.Open(inGridPath, MapWinGIS.GridDataType.UnknownDataType, true, MapWinGIS.GridFileType.UseExtension, callback))
            {
            ...
            foreach (MovingWindow win in inWindows)
            {
                win.WindowCenterRow = maxWinHeight % 2;
            }
            try
            {
                for (int row = 0; row < nr; row++)
                {
                    if (callback != null)
                    {
                        Prog = (int)((row * 100) / (nr));
                        MapWinUtility.Logger.Progress("Calc Window..." + Prog.ToString() + "% Complete", Prog, OldProg);
                        callback.Progress("Progress", Prog, "Calc Window Row: " + (row + 1).ToString());
                    }

                    //Get the rows above the center row. Window row size is always odd, so mod 2 works as a limit as well as a reverse counter
                    for (int i = 0; i < maxWinHeight % 2; i++)
                    {
                        currRealRow = row - ((maxWinHeight % 2) - i);
                        if (currRealRow < 0)
                        {
                            rowBuffer[i] = new float[nc];
                        }
                        ...
                        {
                            inGrid.GetRow(currRealRow, ref rowBuffer[i][0]);
                        }
                    }
                    
                    //Get the center row
                    inGrid.GetRow(row, ref rowBuffer[maxWinHeight % 2][0]);
                    
                    //Get the rows below the center row
                    for (int i = maxWinHeight % 2 + 1; i < maxWinHeight; i++)
                    {
                        currRealRow = row + (i - (maxWinHeight % 2));
                        if (currRealRow > nr)
                        {
                            rowBuffer[i] = new float[nc];
                        }
    ...
    public class MovingWindow : Object
    {
        ...
        protected object _NoDataValue = -1;
        ...
        public bool Initialize(string OutPath, MapWinGIS.GridHeader InHead)
        {
            ...
            if (OutPath != "" && InHead != null)
            {
                ...
                outHead.NodataValue = float.MinValue;
                ...
                Result = _OutGrid.CreateNew(OutPath, outHead, MapWinGIS.GridDataType.FloatDataType, outHead.NodataValue, true, MapWinGIS.GridFileType.UseExtension, null);
                if (Result == false)
                {
                ...
                _NoDataValue = outHead.NodataValue;
            }
            else
            {
