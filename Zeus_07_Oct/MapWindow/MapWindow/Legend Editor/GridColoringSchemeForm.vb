...
#End Region
    ...
    Public Shared Function GetUniqueBreaks(ByRef grd As MapWinGIS.Grid, ByVal Silent As Boolean, ByRef ColorScheme As MapWinGIS.GridColorScheme, ByVal gradientModel As MapWinGIS.GradientModel, ByVal coloringType As MapWinGIS.ColoringType, ByVal NumberFormat As String, ByVal Precision As Integer) As Boolean
        Dim i, j As Integer
            ...
            For j = 0 To grd.Header.NumberCols - 1
                ...
                If ht.ContainsKey(val) = False Then
                    ...
                    If ht.Count > 100 Then
                        If Not Silent Then MapWinUtility.Logger.Msg("There are too many unique values to create this scheme.", MsgBoxStyle.Information Or MsgBoxStyle.OkOnly, "Coloring Scheme Editor")
                        Return False
                    End If
                End If
            Next
        ...
        If ht.Count > 100 Then
            If Not Silent AndAlso MapWinUtility.Logger.Msg("There are " & ht.Count & " unique values.  Are you sure you want to continue?", MsgBoxStyle.YesNo, "Continue?") = MsgBoxResult.No Then
                Return False
            End If
        End If
        ...
        While ColorScheme.NumBreaks > 0
            ColorScheme.DeleteBreak(0)
        End While
        ...
        For i = 0 To arr.Length - 1
            'This must be double.parse(convert.tostring) so that it can
            'handle SByte values also. CDM 11/13/2005
            '            If CDbl(arr(i)) <> CDbl(grd.Header.NodataValue) Then
            If Double.Parse(Convert.ToString(arr(i))) <> Double.Parse(Convert.ToString(grd.Header.NodataValue)) Then
                brk = New MapWinGIS.GridColorBreak
                brk.LowColor = System.Convert.ToUInt32(Rnd() * Integer.MaxValue)
                brk.HighColor = brk.LowColor
                'Use double.parse(convert.tostring) so that SByte can be
                'handled as well. CDM 11/13/2005
            ...
            If ht.Keys.Count <= numDesired Then
                ...
                For i = startVal To brkArr.Length - 1
                    ...
                    If IsNumeric(brkArr(i)) Then
                        brk.Caption = Double.Parse(Convert.ToString((brkArr(i)))).ToString(m_NumberFormat & m_Precision)
