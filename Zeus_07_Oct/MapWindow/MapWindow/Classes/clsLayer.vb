...
Public Class Layer
    ...
    Public Sub HatchingRecalculate() Implements MapWindow.Interfaces.Layer.HatchingRecalculate
        If Not frmMain.m_FillStippleSchemes.Contains(m_LayerHandle) OrElse frmMain.m_FillStippleSchemes(m_LayerHandle) Is Nothing OrElse CType(frmMain.m_FillStippleSchemes(m_LayerHandle), MapWindow.Interfaces.ShapefileFillStippleScheme).NumHatches = 0 Then
    ...
    Function LoadShapeLayerProps() As Boolean Implements Interfaces.Layer.LoadShapeLayerProps
        If m_LayerHandle = -1 Then Return False

        Return frmMain.LoadShapeLayerProps(m_LayerHandle)
    End Function

    Function LoadShapeLayerProps(ByVal loadFromFilename As String) As Boolean Implements Interfaces.Layer.LoadShapeLayerProps
        If m_LayerHandle = -1 Then Return False

        Return frmMain.LoadShapeLayerProps(m_LayerHandle, loadFromFilename)
    End Function
    ...
    Public Property PointImageScheme() As Object Implements Interfaces.Layer.PointImageScheme
        Get
            If frmMain.m_PointImageSchemes.Contains(m_LayerHandle) Then
                Return frmMain.m_PointImageSchemes(m_LayerHandle)
            End If
            Return Nothing
        End Get
        Set(ByVal value As Object)
            If frmMain.m_PointImageSchemes Is Nothing Then frmMain.m_PointImageSchemes = New Hashtable
            If frmMain.m_PointImageSchemes.Contains(m_LayerHandle) Then
                frmMain.m_PointImageSchemes(m_LayerHandle) = value
            Else
                frmMain.m_PointImageSchemes.Add(m_LayerHandle, value)
            End If
        End Set
    End Property
    ...
    Public Property HideFromLegend() As Boolean Implements Interfaces.Layer.HideFromLegend
        Get
            If m_LayerHandle = -1 Then Return False
            If frmMain.Legend.Layers(m_LayerHandle) Is Nothing Then Return False

            Return frmMain.Legend.Layers(m_LayerHandle).HideFromLegend
        End Get
        Set(ByVal value As Boolean)
            If m_LayerHandle = -1 Then Return
            If frmMain.Legend.Layers(m_LayerHandle) Is Nothing Then Return

            frmMain.Legend.Layers(m_LayerHandle).HideFromLegend = value
        End Set
    End Property
    ...
    Public ReadOnly Property LayerType() As Interfaces.eLayerType Implements Interfaces.Layer.LayerType
        Get
            If m_LayerHandle = -1 Then
                m_error = "Layer object not initialized with a valid handle"
            Else
                Return GetLayerType(m_LayerHandle)
            End If
        End Get
    End Property
    ...
    Public Property UseTransparentColor() As Boolean Implements Interfaces.Layer.UseTransparentColor
        ...
        Set(ByVal Value As Boolean)
            ...
            If Value <> img.UseTransparencyColor Then
                ...
                frmMain.Legend.Layers(m_LayerHandle).Refresh()
            End If
