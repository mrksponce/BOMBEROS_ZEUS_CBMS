...
{
    public class TemplatePluginVS2008Plugin : MapWindow.Interfaces.IPlugin
    {
        private MapWindow.Interfaces.IMapWin _mapWin;
        private int _parentHandle;
        private TemplatePluginVS2008.Forms.frmLayers frm;

        //multiple buttons/plug-ins can be placed on the same toolbar
        //Change these values to your own needs:
        private string _toolbarName = AssemblyInformation.ProductCompany();
        private const string _btnNameLoadData = "TemplatePluginVS2008Plugin"; //This need to be a constant
        private string _toolTipLoadData = AssemblyInformation.ProductDescription();
        ...
        {
            if (UserActivatedPlugin && (msg.ToUpper().Contains("LAYERVISIBLECHANGED")))
            {
                //Re-initialize form:
                if (frm != null)
                    frm.clsPlugin.fillVisibleLayers(frm);
            }
...
#region Assembly stuff
        ...
        public string Version
        {
            get
            {
                ...
                var f = System.Diagnostics.FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location);
